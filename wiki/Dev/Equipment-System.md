...
- **Gear** discovery covers NVGs (`ItemInfo` type 617), plain binoculars (excluding rangefinders/designators, which carry a battery magazine and get classified as weapons instead), the highest-armor vest found, and the first explosive throwable found among the `Throw` weapon's muzzles.
...
`buildArsenal` writes its results straight to `missionNamespace` as globals the rest of the mission reads directly: `lootPriWeapons` / `lootSecWeapons` / `lootAttachments` (ground loot), `airdropLoadouts`, `TraitorRifle`/`*Mag`/`*Optics`, `TraitorLauncher`/`*Mag`, `ShopPistol`/`*Mag`/`*Suppressor`, `ShopArmorVest`, `ShopFrag`, `ShopNVG`, `ShopBinocular`, `uniformsConfig` / `headgearsConfig` / `vestsConfig`, and `detectiveConfig`.
